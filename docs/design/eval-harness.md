...
M2 落成（#9）注：校准框架按本节实现于 `eval/calibration/`——判据（rubric、阈值、协议 commit、算法）为固定内容并取哈希入记录；抽样清单由两个 dev 侧 run 产物生成（固定种子，dev 侧抽取、分层按题型与拒答）；worksheet 九列即标注者可见的全部内容（无条件标签、无 judge 结论、无证据）；不可解析输出按评分阶段失败语义单独计数，不入一致率分母也不默认判错；判定记录携带 judge 四项标识与运行日期，判据未变时重算判定逐字节一致。不达标（含未附记录的真实 judge run）把问答结论降级为诊断项并标注与官方 GPT-4o 的偏离，compare 同步移入诊断通道。人工盲标是用户环节，操作步骤见 [judge-calibration-workflow](judge-calibration-workflow.md)。
...
| M2：真实开发集基线 | M1 完成；按已确定事项加载数据版本与许可证记录、reader/judge 配置、评分协议与 tokenizer，并固定仍待定的运行参数 | 数据下载与校验说明、数据清单、50/450 划分清单、judge 校准记录（rubric、抽样清单、标注、一致率与阈值判定）、三种基线 adapter、官方评分接入、50 题基线报告及比较结果 | 两个划分无重叠且覆盖 500 题；judge 校准在 100 条随机样本上完成并给出明确的阈值判定；三种基线都有完整报告，上下文超限可追踪；每题达到明确终态，成绩包含既定分母、状态、覆盖率和资源信息（#8 已落成：真实 reader/judge 接入、三种基线、精确计数与校准、上下文预检、四项版本标识与探测留档、smoke-live 与 dev50 示例配置；#9 已落成：judge 校准框架——固定判据、100+20 抽样与脱敏标注、一致率/Wilson/混淆矩阵/阈值判定、判定记录与降级诊断通道、live 批量调用脚本与人工标注工作流文档；剩余为凭证就绪后的实际校准执行与 50 题正式报告） |
...
| Judge 偏离官方模型 | 按“Judge 校准”执行 100 条随机样本与 20 条边界样本的判定；报告总体一致率与 Wilson 区间、混淆矩阵、分题型与拒答子集结果、自身一致率与跨条件差；标明与官方 GPT-4o 的偏离，不宣称与论文分数可比（#8 落成协议侧：anscheck 模板与解析语义逐字绑定上游 commit 9e0b455f、偏离随版本记录留档；#9 落成校准侧：100+20 抽样/统计/阈值判定/记录 + 不可解析输出按评分阶段失败语义排除 + 不达标降级诊断与 GPT-4o 偏离标注接入 report/compare 诊断通道；实际盲标与判定产出随用户执行） |
...
| Judge 校准方案 | 100 条随机样本（开发集 50 题 × 两个条件的输出，分层覆盖六题型与拒答）加 20 条边界样本；单人盲标，抽 20 条间隔复标测自身一致性；阈值：总体 ≥0.85 且 Wilson 95% 下界 ≥0.75，任一题型 <0.70 单独标注，拒答同总体，跨条件差 ≤0.05 | 用户确认；Wilson 区间按 n=100 估计（#9 已落成框架：`eval/calibration/` 固定判据（rubric/阈值/协议 commit 内容哈希）与抽样、脱敏 worksheet、统计与判定、记录（judge 四项标识+运行日期+判据哈希+判定稳定校验）；一键脚本 `scripts/judge_calibration.py` plan/judge/report；不达标经 `judge_calibration` 诊断块把问答结论降级并接入 compare 诊断通道；人工盲标执行见 docs/design/judge-calibration-workflow.md） |
